...
| 题目 | 核心思维模型 | 可以解决哪类问题？ |
| :--- | :--- | :--- |
| **167. 两数之和 II** | **对撞指针** (两端收缩) | 有序数组中寻找特定目标值（两数之和、三数之和等）。 |
| **11. 盛最多水的容器** | **贪心缩界** (对撞) | 在有序或由边界决定的数组中，寻找受两端约束的极值（最大面积、存水等）。 |
| **26. 删除重复项** | **快慢指针** (原地修改) | 在线性结构中进行原地删除、去重、或寻找环路/中点。 |

## 6. 行动指南

我们将按照循序渐进的顺序，开始双指针的实战练习：
1.  **[167. 两数之和 II - 输入有序数组]** - 体验“对撞指针”的边界收缩。
2.  **[11. 盛最多水的容器]** - 巩固“对撞指针”在面积极值问题中的贪心应用。
3.  **[142. 环形链表 II]** - 认识“快慢指针”在成环结构中的物理规律。
4.  **[26. 删除有序数组中的重复项]** - 练习“快慢指针”的原地修改套路。

## 7. 学术语对照：大纲里的关键词都在哪？

| 大纲关键词 | 我们的实战模型 | 对应你代码中的动作 |
| :--- | :--- | :--- |
| **有序** | **二分先验条件** | 通常先对数组进行 `sort()`，利用大小排列产生的确定性。 |
| **对撞排除** | **两端夹逼 (Collision)** | `left` 向右或 `right` 向左移动，逻辑上瞬间删掉无效搜索区间。 |
| **快慢探测** | **跑道套圈 / 原地坑位** | `fast` 寻找新面孔或跑得更快，`slow` 守住记录位置。 |

---
*如果你已经准备好在纸上画出两个指针的对撞过程，请告诉我。我们将开启双指针的首战：**167. 两数之和 II (有序数组)****。*
