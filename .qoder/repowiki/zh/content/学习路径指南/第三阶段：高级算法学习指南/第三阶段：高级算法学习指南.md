...
- [solutions/stage3/dp/house_robber_198.py](file://solutions/stage3/dp/house_robber_198.py)
...
- solutions/stage3/dp：动态规划的经典问题实现，包括爬楼梯、最小花费爬楼梯、打家劫舍等DP问题
...
K --> N["选择型DP<br/>house_robber_198.py"]
```
...
```mermaid
flowchart TD
Start3(["开始"]) --> Init3["初始化prev1=0, prev2=0"]
Init3 --> Loop3["遍历i从1到n"]
Loop3 --> Choice{"选择当前房屋?"}
Choice --> |否| NoPick["dp[i] = dp[i-1]"] --> Update3["更新状态"]
Choice --> |是| YesPick["dp[i] = dp[i-2] + nums[i]"] --> Update3
Update3 --> Next3["继续下一个i"]
Next3 --> Loop3
Loop3 --> |完成| Return3["返回max(prev1, prev2)"]
```
...
H["选择型DP<br/>house_robber_198.py"] -.典型.-> B
```
