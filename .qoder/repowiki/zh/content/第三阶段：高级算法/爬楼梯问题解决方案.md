...
## 简介
本仓库聚焦于“爬楼梯”相关问题的动态规划解法，包含两道经典题目：
- 爬楼梯（第70题）：每次可走1或2阶，求到达第n阶的不同方法数。
- 使用最小花费爬楼梯（第746题）：每步有代价，可从索引0或1出发，求到达顶部（索引n）的最小总代价。

文档从系统架构、数据流与处理逻辑等维度进行拆解，并给出可视化图示与排障建议，帮助读者快速理解与复用代码。
...
B --> C["stage3"]
C --> D["dp"]
D --> E["climbing_stairs_70.py"]
D --> F["min_cost_climbing_stairs_746.py"]
```
...
## 架构总览
两个模块均为独立脚本，各自提供函数式接口，无外部依赖耦合，便于单独测试与集成。

```mermaid
graph TB
subgraph "动态规划模块"
M1["climbing_stairs_70.py<br/>函数：爬楼梯(n)"]
M2["min_cost_climbing_stairs_746.py<br/>函数：最小花费爬楼梯(cost)"]
end
U["调用方/评测器"] --> M1
U --> M2
```
...
## 结论
本仓库的两份实现分别覆盖了“计数型”和“最小型”两类爬楼梯问题，均采用标准动态规划范式，并通过滚动变量实现常数空间优化。代码结构清晰、职责单一，易于扩展与复用。
